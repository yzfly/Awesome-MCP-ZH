...
| [whw23/searxng-http-mcp](https://github.com/whw23/searxng_http_mcp) | 自包含 SearXNG 元搜索 MCP 服务器。200+ 搜索引擎，30+ 分类，多页并行，自动补全。Docker 一键部署，支持 HTTP/stdio/uvx 三种传输模式，内置 API Key 认证和 Web UI 反向代理。Claude Code 插件含可信度验证、AI 投毒防御和交叉验证能力。 | 社区实现, Python 开发 🐍, 本地运行 🏠, 云服务 ☁️, SearXNG 元搜索（自包含 Docker）。 |
