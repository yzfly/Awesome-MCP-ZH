...
| [APITube News MCP](https://github.com/apitube/news-api-mcp) | 托管式新闻检索服务，覆盖 50 万+ 新闻源、177 个国家和 59 种语言，中文来源可直接用中文关键词检索。每篇文章返回结构化字段：情感分值、实体标注（人物、机构、地点、品牌、事件）、IPTC 分类，可按标题关键词、语种、国家、来源域名与质量排名、时间区间、情感区间筛选并排序。 | 官方实现 (APITube) 🎖️, 云服务 ☁️, 远程 Streamable HTTP `https://mcp.apitube.io/`，免安装，`Authorization: Bearer <API_KEY>` 或 `X-API-Key` 认证；2 个工具（`search_news`、`suggest`），单次最多返回 25 篇；工具调用需付费套餐（$29/月起），MIT。 |
