...
| [MCP Doctor](https://github.com/xlyoung/mcp-doctor) | MCP Server 安全扫描与质量评分 CLI 工具。8 项安全检测引擎（提示注入、路径遍历、SSRF、凭据泄露等），质量评分（0-100），内置 100+ MCP Server 注册表，一键安装并自动拦截高危漏洞。 | 社区实现, Python 开发 🐍, 本地运行 🏠, 跨平台 🍎🪟🐧, MCP Server 安全扫描与质量评估。 |
