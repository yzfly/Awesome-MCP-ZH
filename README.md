...
| [eltociear/skill-audit-mcp](https://github.com/eltociear/skill-audit-mcp) | MCP 服务器、AI 代理技能与插件的静态安全扫描器。检测 68 个攻击模式，覆盖 4 个严重级别（CRITICAL/HIGH/MEDIUM/LOW）：凭证泄露、提示注入、代码执行、助记词收集、认证绕过、路径穿越。零依赖、SARIF 输出可直通 GitHub Code Scanning，并发布为 GitHub Action（`uses: eltociear/skill-audit-mcp@v1`）。 | 社区实现, Python 开发 🐍, 本地/CI 运行 🏠, 跨平台 🍎🪟🐧, MCP 安全扫描 + GitHub Action。 |
