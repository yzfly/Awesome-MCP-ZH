...
| [Awareness](https://github.com/everest-an/Awareness-Market)                      | 面向编码 Agent 的本地优先持久记忆。在 LongMemEval（ICLR 2025，500 道人工标注题）上 Recall@5 达 96.0%，且检索阶段零 LLM 调用（BM25 + 向量 RRF 混合）。完全离线运行，无需账号与 API Key。`npx @awareness.market/local start` |
