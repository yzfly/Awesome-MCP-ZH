...
| [Beever Atlas](https://github.com/Beever-AI/beever-atlas)                         | 开源团队知识库，内建 MCP 服务器（16 个工具）。Neo4j 类型化知识图谱 + Weaviate 向量数据库。支持 Slack/Discord/Teams/Telegram 对话导入。通过 LiteLLM 自选 LLM。Apache 2.0 授权，支持本地部署。 | 社区实现, Python 开发 🐍, 本地运行 🏠, Neo4j 知识图谱 + Weaviate 向量搜索, Apache 2.0。 |
