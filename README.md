...
| [Contexter](https://github.com/maaakso/contexter-mcp)                          | 一个项目上下文，供 ChatGPT、Claude、Telegram 及任意 MCP 客户端共用，而不是每个工具各存一份。在一个助手里保存结果，在另一个助手里继续，还可以把整个上下文分享给他人，对方用自己的 AI 打开。不会自动导入聊天记录，只有你明确保存的内容才会进入上下文。 | 官方实现 (Contexter) 🎖️, 云服务 ☁️, 远程 Streamable HTTP + OAuth 2.1（支持动态客户端注册，只填 URL 即可连接），26 个工具，`https://contexterai.com/mcp`，已登记于官方 MCP Registry。 |
