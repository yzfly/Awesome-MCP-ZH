...
| [Melaya](https://melaya.org/en/product/mcp) | 远程 MCP 服务器，让 AI 直接操作你自己的**安卓手机**：通过 Android 无障碍（Accessibility）服务读取屏幕节点树，再执行点击、输入、滑动与应用间跳转。不依赖任何单个 App 的接口或厂商 API，因此在暴露无障碍节点的应用里都能用。同时支持操作已配对的浏览器，以及运行可访问 6000+ 工具的 Agent 流水线。权限边界：只能操作用户在白名单中勾选的应用；以用户本人身份发布内容前，会先在手机上弹出确认卡片，用户批准后才会发出。 | 官方实现, 远程 ☁️, Streamable HTTP, OAuth 2.1 + PKCE + 动态注册（无需 API Key，也无需填任何配置）, 端点 `https://api.melaya.org/mcp`, 手机端需安装 Melaya 安卓应用并开启无障碍服务, Apache-2.0。 |
