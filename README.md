...
| [NotFair](https://github.com/nowork-studio/NotFair) | 面向 Claude Code 的开源营销 Agent 技能集，涵盖 SEO（站点分析、关键词研究、元标签、Schema 标记、GEO 优化、内容写作）、Google Ads（审计、浪费消耗检测、搜索词整理、关键词与出价管理）和 Meta Ads（ROAS 分析、素材疲劳检测、受众重叠）。通过 Google Ads MCP、Meta Ads MCP、Google Search Console MCP 和 Google Analytics (GA4) MCP 连接实时数据。 | 社区实现, 本地运行 🏠, Claude Code 营销 Agent 技能集, ~2.9k Stars, MIT 许可。                     |
