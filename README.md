...
| [teehooai/spidershield](https://github.com/teehooai/spidershield) | MCP 服务器安全扫描 & 运行时防护工具。检测46种漏洞模式（反弹Shell、凭证窃取、Prompt注入、SSRF），评估工具描述质量（7维评分），并提供运行时策略引擎 + DLP + 审计日志。类似 "npm audit for MCP tools"。 | 社区实现, Python 开发 🐍, 本地运行 🏠, MCP 服务器安全审计。 |
