...
| [TwitterAPI.io MCP Server](https://github.com/kaitoInfra/twitterapi-io-mcp-server) | 官方 MCP 服务器，对接 [twitterapi.io](https://twitterapi.io)（Twitter/X 数据 API）。12 个只读工具：推文搜索（支持完整搜索操作符）、用户档案、关注者、对话线程、实时 WebSocket 流、趋势话题、互动指标。托管端点 `mcp.twitterapi.io/mcp`，npm `@kaitoinfra/twitterapi-io-mcp-server`。 | 官方实现 🎖️, TypeScript 开发 📇, 云服务 ☁️, X/Twitter 数据 API 集成。 |
