...
| [Bortlesboat/bitcoin-mcp](https://github.com/Bortlesboat/bitcoin-mcp)                  | 最全面的比特币 MCP 服务器。49 个 AI 代理工具：费用估算、内存池分析、区块/交易检查、挖矿统计、价格和供应数据。零配置即用。已收录于 Anthropic MCP 官方注册表。 | 社区实现, Python 开发 🐍, 云服务 ☁️, 本地运行 🏠, 比特币网络全面分析 ₿。                                  |
