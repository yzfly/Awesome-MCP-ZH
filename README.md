...
| [nowork-studio/toprank](https://github.com/nowork-studio/toprank) | NotFair Google Ads MCP 服务器：连接 Claude 和 AI 代理到您的 Google Ads 账户，诊断广告系列性能（CPA、ROAS、搜索词浪费、质量得分），推荐优化（出价、预算、否定关键词、广告文案），并通过 Google Ads API 执行已批准的更改（内置人工审批门）。 | 官方实现 (NotFair) 🎖️, 云服务 ☁️, 流式 HTTP 端点 `notfair.co/api/mcp/google_ads`, 已在官方 MCP 注册表登记 (`io.github.nowork-studio/notfair`)。 |
