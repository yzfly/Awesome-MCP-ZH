...
| [BuyWhere/buywhere-mcp](https://github.com/BuyWhere/buywhere-mcp) | 面向 AI 代理的跨境电商产品搜索与价格比较 API。覆盖 1100 万+ 商品（新加坡、东南亚、美国），3 秒自助 API 密钥、零注册。提供 `search_products`、`find_similar`、`get_product`、`find_deals`、`get_catalog` 等工具。 | 官方实现 🎖️, TypeScript 开发 📇, 云服务 ☁️, 跨境电商 (新加坡/东南亚/美国)。 |
