...
| [CumpsD/broker-as-a-service](https://github.com/CumpsD/broker-as-a-service) | 基于 Chainflip 协议的跨链兑换 MCP 服务器,支持报价、简单或 DCA 兑换,以及兑换进度跟踪,覆盖原生 BTC、ETH、SOL 等资产。 | 官方实现 🎖️, 云服务 ☁️, 免 API 密钥,远程服务器 `https://chainflip-broker.io/mcp`。 |
...
| [BulkTranscripts](https://github.com/pratie/bulktranscripts-mcp) | 远程 MCP 服务，批量获取 YouTube 视频字幕/转录文本，并支持视频与频道搜索、频道内搜索、频道视频列表、播放列表提取，以及免费的新上传追踪（RSS）。单次可批量提取 20 个视频，支持纯文本或带时间戳的分段；已获取过的字幕再次读取免费。共 7 个工具。 | 官方实现 (BulkTranscripts) 🎖️, 云服务 ☁️, 远程端点 `https://bulktranscripts.co/mcp` (Streamable HTTP), 无需注册即可使用（每 IP 30 条免费额度）, 可选 `Authorization: Bearer <license key>`, MIT。 |
