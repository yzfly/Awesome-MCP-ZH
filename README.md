...
| [OmniSim MCP](https://github.com/omnilink-tech/omnisim/tree/main/packages/omnisim-mcp) | 为机器人仿真开发提供本地 MCP 接口：智能体可加载或安全热同步世界、检查场景与机器人/关节/接触状态、自动取景并截图、推进或重置仿真，以及读取控制器与物理事件。 | OmniLink 官方实现 🎖️, Python 开发 🐍, stdio / 本地运行 🏠, 零 Python 运行时依赖；需先启动本地 OmniSim World Harness；当前仅支持源码安装（尚未发布至 PyPI）, Apache-2.0。 |
