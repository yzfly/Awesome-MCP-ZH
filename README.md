...
| [Uni-CLI](https://github.com/olo-dot-io/Uni-CLI)                                | AI Agent 通用 CLI，内置 MCP 服务器（`unicli mcp serve`）。756 个命令覆盖 167 个站点，包括丰富的中文平台支持（B站、微博、知乎、小红书、抖音、豆瓣、即刻等 17+ 个中文站点）。自修复 YAML 适配器，~80 tokens/次调用。 | 社区实现, TypeScript 开发 📇, 本地运行 🏠, 跨平台 🍎🪟🐧, 多站点 Agent 基础设施。 |
