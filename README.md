...
| [SandBase Harness](https://github.com/sandbaseai/sandbase-harness) | 本地优先、可自托管的 AI Agent 运行时与 MCP 服务器，提供持久化会话、沙箱工具、Memory、权限审批、审计回放和本地 Console，可接入 OpenAI、Anthropic、MiniMax 及 OpenAI-compatible 模型。 | 官方实现 (SandBase) 🎖️, TypeScript 开发 📇, 本地运行 🏠, 跨平台 🍎🪟🐧, Docker 🐳, Apache-2.0, [官方 MCP Registry](https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.sandbaseai%2Fsandbase-harness)。 |
