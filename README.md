...
| [twzrd-agent-intel](https://intel.twzrd.xyz) | Solana 链上代理信任评分 MCP 服务器，通过 x402 微支付提供服务。4 个免费工具对任意 Solana 钱包进行信任预检（链上活动、交易模式、历史行为）；4 个付费工具（HTTP 402 + USDC on Solana，结算 <1s）返回签名 `twzrd.receipt.v5` 信任令牌，用于自主代理交易验证。MCP 注册表: `xyz.twzrd.intel/twzrd-agent-intel`。 | 社区实现, Python 开发 🐍, 云服务 ☁️, Solana 代理信任评分, x402 微支付。 |
