...
| [Nucleus MCP](https://github.com/eidetic-works/nucleus-mcp)                        | 开源 MCP 服务器，提供 114 个工具，支持持久化记忆、执行验证、治理和合规。GROUND 验证执行，ALIGN 捕获修正，COMPOUND 生成训练信号。本地优先，零云依赖。 | 社区实现, Python 开发 🐍, 本地运行 🏠, 持久记忆 + 执行验证 + 治理。                                  |
