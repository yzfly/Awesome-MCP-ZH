...
| [MCP Observatory](https://github.com/KryptosAI/mcp-observatory) 🔍 | 在 AI Agent 依赖 MCP 服务器之前对其进行测试、安全扫描和监控。支持自动发现、schema 漂移检测、SARIF 集成、录制/回放/验证以及 GitHub Code Scanning。 | 社区实现, TypeScript 开发 📇, 本地运行 🏠, MCP 安全测试与审计, MIT, `npx @kryptosai/mcp-observatory`。 |
