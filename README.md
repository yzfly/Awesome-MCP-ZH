...
| [LeonTing1010/tap](https://github.com/LeonTing1010/tap) | 将 AI 浏览器自动化编译为可确定回放的 `.tap.json` 计划（运行时零 LLM token），通过 Chrome 扩展使用用户已登录的浏览器会话，语义指纹差异检测站点漂移。70+ 开源社区 tap。 | 社区实现, TypeScript/Deno 开发 📇, 本地运行 🏠, 内置 forge/doctor/heal 完整生命周期。 |
