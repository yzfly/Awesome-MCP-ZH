...
| [ezequiellich44-cmd/MandateGuard](https://github.com/ezequiellich44-cmd/MandateGuard) | 确定性支付策略引擎——在任何资金操作执行前强制检查预算、允许/拒绝清单、速率窗口与 Ed25519 签名授权，决策路径中无 LLM（OWASP LLM08 过度代理护栏）。附带防篡改 SHA-256 审计账本，已上架官方 MCP Registry。 | 社区实现, Python 开发 🐍, 本地运行 🏠, 跨平台 🍎🪟🐧, 支付安全与策略执行, MIT 核心。 |
