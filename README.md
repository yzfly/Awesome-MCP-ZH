...
| [Cohesivity](https://github.com/cohesivity-org/cohesivity-plugin) | 面向智能体的后端服务，可通过 MCP 或 API 以匿名账户免注册开始使用，提供托管、Postgres、邮件、存储、容器、LLM、语音和第三方 API，包含免费额度、每月 5 美元 AI 与搜索额度，支持 x402 充值。 | 官方实现 🎖️, JavaScript 开发, 云端/本地, 本地 MCP 需 Node 18+、免注册；远程 MCP `https://cohesivity.ai/mcp/manage` 需 OAuth 登录；插件包含 MCP 和 Skill。 |
