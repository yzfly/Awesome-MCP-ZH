...
| [estevecastells/domscan-mcp](https://github.com/estevecastells/domscan-mcp)          | DomScan 域名情报 MCP 服务器，提供 1500+ 顶级域名的可用性查询、DNS、WHOIS/RDAP、SSL/TLS、子域名枚举、域名估值、邮件安全 (SPF/DKIM/DMARC) 以及仿冒域名/品牌保护检测。 | 社区实现, 远程端点 ☁️ (`https://domscan.net/mcp`, Bearer 鉴权), 域名情报与反钓鱼分析 (DomScan)。 |
