...
| [车来了 MCP (chelaile-mcp)](https://github.com/PeanutSplash/chelaile-mcp)             | 基于车来了 API 的实时公交 MCP 服务器，让 LLM 查询国内公交/地铁实时数据，包括到站时间、车辆位置、线路时刻表、附近站点和换乘规划。无需登录开箱即用。 | 社区实现, TypeScript 开发 📇, 本地运行 🏠, 国内公交/地铁实时数据 (车来了)。                  |
