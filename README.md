...
| [Cross-Validated Search](https://github.com/wd041216-bit/cross-validated-search) | 面向 AI Agent 的证据感知搜索与事实核验工具，支持网页搜索、页面阅读、claim verification 与 citation-ready evidence report。默认可用 `ddgs`，推荐免费增强路径为 `ddgs + 自托管 SearXNG`。安装：`pip install cross-validated-search` | 社区实现, Python 开发 🐍, 本地运行 🏠, 零成本证据检索与核验。 |
