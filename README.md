...
| [AccInt](https://github.com/maxbaluev/accreted-intelligence)                       | 本地优先的 AI 编程代理记忆与 Work Model MCP 服务器，让 Claude Code、OpenCode、Codex、Cursor 共享同一个 SQLite 记忆基底，并把执行结果反馈到后续检索。 | 社区实现, 本地运行 🏠, 跨工具编程代理记忆, MCP Server。                                          |
